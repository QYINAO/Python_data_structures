...
class Printer:
    ...
    def tick(self):  
        if self.currentTask != None:              
            self.timeRemaining = self.timeRemaining - 1   # 变为忙碌状态
            if self.timeRemaining <= 0:
                self.currentTask = None    #变为空闲状态
    ...
    def startNext(self,newTask):                # 进入下一个任务
...
class Task:
    ...
    def getStamp(self):        # 当前时间戳
        return self.timestamp
    
    def getPages(self):    # 当前页数
        return self.pages
...
def simulation(numSeconds,pagesPerMinute):
    ...
    for currentSecond in range(numSeconds):
        ...
        if (not labprinter.busy()) and (not printQueue.isEmpty()):   # 打印机不是空闲状态，且队列不为空
