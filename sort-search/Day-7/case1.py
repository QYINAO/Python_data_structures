...
#     for passnum in range(len(alist)-1,0,-1):   # 反向遍历（8,7,6,5,4...）
#         for i in range(passnum):    # （0，passnum）
#             if alist[i] < alist[i+1]:
#                  # alist[i] 和 alist[i+1]借助变量temp进行交换
#                 temp = alist[i]
#                 alist[i] = alist[i+1]
#                 alist[i+1] = temp
...
#     for fillsort in range(len(alist)-1,0,-1):   # 列表长度
#         maxPos = 0
...
'''
    插入排序     为每个元素找合适位置（大于前一个元素，小于后一个元素）
'''
...
'''
    希尔排序    将列表分割成若干子列表，先对【子列表内】插入排序，再对【子列表】进行插入排序。
'''
# def shellSort(alist):
